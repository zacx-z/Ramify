...
namespace Nela.Ramify {
    ...
    public class ViewTreeWindow : EditorWindow, IViewTreeWindow {
        public static void OpenViewTreeNode(View viewNode) {
            OpenViewTree(FindRoot(viewNode), viewNode);
        }

        public static ViewTreeWindow OpenViewTree(View root, View selectedNode) {
            var viewer = EditorWindow.GetWindow<ViewTreeWindow>();
            viewer.Open(root, selectedNode);
            return viewer;
        }
        ...
        private void Open(View root, View selectedNode) {
            _root = root;
            var selected = selectedNode ?? root;

            if (_viewTreeView != null) {
                _viewTreeView.SetViewRoot(root);
                _viewTreeView.SetSelection(new [] { selected.GetInstanceID() });
                _viewTreeView.ExpandToShow(selected);
            }

            UpdateInfoPanel(new[] { selected });
        }
